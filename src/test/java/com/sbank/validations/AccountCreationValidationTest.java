...
public class AccountCreationValidationTest {
    ...
    public void givenNullNameThenNewAccountRequestValidationFails() {
        singleFieldFailureCase(new NewAccountRequest(null, "Palm Jumairah, Dubai, UAE", BigDecimal.TEN), "name", MSG_MUST_NOT_BE_EMPTY);
    }

    @Test
    public void givenEmptyNameThenNewAccountRequestValidationFails() {
        singleFieldFailureCase(new NewAccountRequest("", "Palm Jumairah, Dubai, UAE", BigDecimal.TEN), "name", MSG_MUST_NOT_BE_EMPTY);
    }
    ...
    public void givenNullAddressThenNewAccountRequestValidationFails() {
        singleFieldFailureCase(new NewAccountRequest("bill gates", null, BigDecimal.TEN), "address", MSG_MUST_NOT_BE_EMPTY);
    }

    @Test
    public void givenEmptyAddressThenNewAccountRequestValidationFails() {
        singleFieldFailureCase(new NewAccountRequest("bill gates", "", BigDecimal.TEN), "address", MSG_MUST_NOT_BE_EMPTY);
    }
    ...
    public void givenNullBalanceThenNewAccountRequestValidationFails() {
        singleFieldFailureCase(new NewAccountRequest("bill gates", "address x", null), "balance", MSG_MUST_NOT_BE_EMPTY);
    }

    @Test
    public void givenNegativeBalanceThenNewAccountRequestValidationFails() {
        singleFieldFailureCase(new NewAccountRequest("bill gates", "address x", BigDecimal.valueOf(-1)), "balance", MSG_MUST_BE_POSITIVE);
    }

    @Test
    public void givenZeroBalanceThenNewAccountRequestValidationFails() {
        singleFieldFailureCase(new NewAccountRequest("bill gates", "address x", BigDecimal.ZERO), "balance", MSG_MUST_BE_POSITIVE);
    }
    ...
    public void givenEmptyNameAndAddressAndBalanceThenNewAccountRequestValidationFailsWithThreeViolations() {
        ...
        assertThat(violations.stream().map(v-> v.getPropertyPath().toString()).collect(Collectors.toSet()))
                .containsExactlyInAnyOrder("name","address","balance");
